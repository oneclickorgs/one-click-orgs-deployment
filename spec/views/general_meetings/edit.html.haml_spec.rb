...
describe 'general_meetings/edit' do
  ...
  it "renders a minutes field for each of the agenda items" do
    ...
    expect(rendered).to have_selector(:label, :for => 'general_meeting_agenda_items_attributes_0_minutes', :content => "Apologies for Absence")
  ...
  it "renders a checkbox for each member" do
    ...
    expect(rendered).to have_selector(:label, :for => 'general_meeting_participant_ids_1', :content => "John Smith")

    expect(rendered).to have_selector(:input, :type => 'checkbox', :name => 'general_meeting[participant_ids][2]')
    expect(rendered).to have_selector(:label, :for => 'general_meeting_participant_ids_2', :content => "Sally Baker")
  end
